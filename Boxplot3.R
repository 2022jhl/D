...
for (s in 1:45) {
  ...
  aa<- a0[which(a0[,2]==10 | a0[,2]==21 | a0[,2]==32 |a0[,2]==50 | a0[,2]==65 ),]
  as.factor(aa[,2]); 
  ...
  pot <- ppp+geom_signif(data=aa,mapping=aes(x=V2,y=V1), # 不同组别的显著性
                         comparisons = list(c("S10", "S21"), # 哪些组进行比较
                                            c("S21", "S32"),
                                            c("S50", "S65")),
  ...
  ggsave(paste("D:\\结果\\1variable_pic_S10\\",ss[s],".jpeg",sep=""), plot=pot2, width = 10, height= 10, units = "cm")
}
